...
class WhatsappApplications():
    def __init__(self):
        ...
        while True:
            try:
                gelenmesaj = self.driver.find_element(By.CSS_SELECTOR, '#main > div._3B19s > div > div._5kRIK > div.n5hs2j7m.oq31bsqd.gx1rr48f.qh5tioqs > div:nth-child(4) > div > div > div > div._1BOF7._2AOIt > div.cm280p3y.to2l77zo.n1yiu2zv.c6f98ldp.ooty25bp.oq31bsqd > div.copyable-text > div').text
                global mesaj
                mesaj = gelenmesaj
...
class Applications():
    def __init__(self):
        try:
            ...
            self.nakit = self.driver.find_element(By.XPATH, '//*[@id="widget1_portfoy"]/div/div/div/div[11]/div[2]').text
            self.nakit1 = self.nakit.split(".")
            self.nakit2 = self.nakit1[0]
            self.nakit3 = self.nakit2.replace(",", ".")
            self.nakit4 = str(int(self.nakit3) * 75 / 100).split('.')
            ...
            if len(hisseList) == 0:
                ...
                $(document).ready(function() {
                    var container = $('.jspContainer');
                    var scrollTo = 100; // Kaydırma miktarı (piksel cinsinden)
                ...
                for div_element in div_elements:
                    ...
                    adet = 0
                    yuzdeartis = float(hisseyuzdelik) + 0.05
                    ...
                    hisseList.append([hisseadi, hissefiyat, hisseyuzdelik, yuzdeartis, yuzdeazalis, adet, hisse1, hesap1, hesap2, i])

            else:
                for i in range(0, len(hisseList)):
                    # İşlemlerinizi burada yapabilirsiniz
                    print(f"|" + "-" * 50)
                    print(f"| [+] Hisse ID : {str(hisseList[i][10])}")
                    ...
                    hesap1 = float((float(float(hisseList[i][1]) / 100) * float(hisseList[i][3])) + float(hisseList[i][1]))
                    hesap1 = "{:.3f}".format(hesap1)
                    hisseList[i][7] = f"{hesap1}"
                    print(f"| [+] Hisse Hedef Artış Fiyat : {hisseList[i][7]}")
                    print(f"| [+] Hisse Hedef Azalış Yüzdelik {hisseList[i][4]}")
                    hesap2 = float(float(hisseList[i][1]) - (float(float(hisseList[i][1]) / 100) * float(hisseList[i][4])))
                    hesap2 = "{:.3f}".format(hesap2)
        ...
        while True:
            try:
                ...
                for div_element in div_elements:
                    ...
                    print("|" + "-" * 72 + "|")
                    print(f"| Hisse Adı: {self.hisseadi} - Hisse Fiyat : {self.hissefiyat}  - Hisse Yüzdelik : {self.hisseyuzdelik}")
                    print("|" + "-" * 72 + "|")
                                ...
                                if int(self.adet) == 0:
                                    ...
                                    try:
                                        ...
                                        try:
                                            ...
                                            print(f"| Hisse Adı : {hisseadi2} - Tane Fiyatı : {self.hissefiyat2} * Satılacak Adet Sayısı : {adetc} : {sonuc}")
                                            ...
                                            hesap1 = (float(float(self.hissefiyat) / 100) * float(hisseList[i][3])) + float(self.hissefiyat)
                                            hesap1 = "{:.3f}".format(hesap1)
                                            hisseList[i][7] = f"{hesap1}"
                                            print(f"| [+] Hisse Hedef Artış Fiyat : {hisseList[i][7]}")
                                ...
                                if int(self.adet) >= 100:
                                    ...
                                    if float(self.nakit) < 10.00:
                                        ...
                                        if self.alinacak_Adet <= 0:
                                            ...
                                            print(f"|                         [+] Alıncak Adet : {self.alinacak_Adet}                          |")
                                            ...
                                            try:
                                                ...
                                                print(f"| Hisse Adı : {hisseadi2} - Tane Fiyatı : {self.hissefiyat2} * Alınacak Adet Sayısı : {self.alinacak_Adet} : {sonuc}")

                                                hisseList[i][5] = int(hisseList[i][5]) + int(self.alinacak_Adet)
                                                adet2 = hisseList[i][5]
                                                hisseList[i][3] = float(hisseList[i][3]) + 0.05
                                                hisseyüzdelikartis = "{:.2f}".format(hisseList[i][3])
                                                hisseList[i][4] = float(hisseList[i][4]) + (-0.15)
                                                hisseyüzdelikazalis = "{:.2f}".format(hisseList[i][4])
                                                print(f"| [+] Adet: {self.alinacak_Adet} satın alma emri verilmiştir.")
                                                print(f"| [+] Hisse Adı : {hisseadi2} - Adet : {adet2} olmuştur.")
                                                print(f"| [+] Hisse Yüzdelik Artış Hedef : {hisseyüzdelikartis} olmuştur.")
                                                hesap1 = float((float(float(self.hissefiyat) / 100) * float(hisseList[i][3])) + float(self.hissefiyat))
                                                hesap1 = "{:.3f}".format(hesap1)
                                                hisseList[i][7] = f"{hesap1}"
                                                print(f"| [+] Hisse Hedef Artış Fiyat : {hisseList[i][7]}")
                                                print(f"| [+] Hisse Yüzdelik Azalış Hedef : {hisseyüzdelikazalis} olmuştur.")
                                                hesap2 = float(float(self.hissefiyat) - (float(float(self.hissefiyat) / 100) * float(hisseList[i][4])))
                                                hesap2 = "{:.3f}".format(hesap2)
                                                hisseList[i][8] = f"{hesap2}"
                                                print(f"| [+] Hisse Hedef Azalış Fiyat {hisseList[i][8]}")
